...
| `ZeroAllocJobScheduler` | Transitive package for approved Arch runtime packages. | Scheduler support used by Arch.System. | Exact central pin. | Transitive module runtime only. | May not be referenced directly by module code unless promoted to an explicit approved direct package. | Resolved package validation. |
...
- `tools/validation/validate_project_reference_boundaries.py` rejects module/basegame references to client, server, old-architecture, or private host projects. Modules may reference shared contracts only.
...
- `tools/validation/Octaryn.ModuleApiProbe/` scans module C# source with Roslyn before compile and rejects denied filesystem, networking, process, reflection, native interop, console, environment, dynamic-loading, and raw-threading APIs. Its denied groups must stay aligned with `DeniedFrameworkApiGroups`.
...
- Old dependency cache paths such as `build/shared/deps/<bucket>` and `logs/deps/<bucket>` -> `build/dependencies/<bucket>` and `logs/build/dependencies/<bucket>` unless a dependency is truly owner-local.
