...
| `old-architecture/source/world/lighting/` | held under `old-architecture/` until a dedicated lighting plan exists | Do not port DDGI, skylight propagation, server lighting contracts, lighting probes, or client lighting rewrites during the non-lighting owner split. Existing basegame `skylightOpacity` remains content metadata only. |
...
| `old-architecture/source/core/persistence/lighting_tuning.cpp` and lighting tuning fields in `scalar_types.h` and `serialization.cpp` | `octaryn-client/Source/Native/Settings/LightingSettings/` | Ported as `octaryn_client_lighting_settings`; these are client presentation settings, not server world-light authority, DDGI, or skylight propagation. |
