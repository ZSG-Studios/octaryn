...
| `old-architecture/source/core/memory_mimalloc.*` | `octaryn-shared/Source/Memory/NativeMemory/` | Focused native memory support; remove or isolate SDL coupling so consumers depend on a support target, not allocator internals. |
| `old-architecture/source/core/profile.h` | `octaryn-shared/Source/Diagnostics/NativeProfiling/` plus owner-specific scopes | Split Tracy/native timing helpers from owner log calls; no generic profiling bucket. |
...
| `old-architecture/source/core/asset_paths.*` | `octaryn-client/Source/Assets/ClientAssetPaths/` | Uses SDL base path and runtime asset layout, so it is client packaging/presentation support. |
| `old-architecture/source/core/render_distance.h` | `octaryn-client/Source/Settings/RenderDistance/` plus shared value contract only if needed | Render distance is a client presentation setting unless a later server contract needs a pure value type. |
| `old-architecture/source/core/camera/` | `octaryn-client/Source/Presentation/Camera/` | Camera matrices, frustum, FOV, and viewport state are client presentation only. |
