...
if [[ "$install_deps" == "1" ]]; then
  bash "$(octaryn_workspace_root)/tools/build/install_deps.sh" --yes
...
for preset in "${profiles[@]}"; do
  ...
  bash "$(octaryn_workspace_root)/tools/build/configure.sh" --preset "$preset"
  ...
  bash "$(octaryn_workspace_root)/tools/build/cmake_build.sh" --preset "$preset" --target "$target"
done
