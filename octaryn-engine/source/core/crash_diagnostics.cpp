...
#define OCTARYN_HAS_CPPTRACE 1
#else
#define OCTARYN_HAS_CPPTRACE 0
#endif
...
constexpr std::size_t kCrashMarkerPathSize = 256;
char g_crash_marker_path[kCrashMarkerPathSize] = {};
...
void write_marker_bytes(const char* message, std::size_t message_size)
{
    ...
    HANDLE marker_file = CreateFileA(g_crash_marker_path,
                                     FILE_APPEND_DATA,
                                     FILE_SHARE_READ,
                                     nullptr,
                                     OPEN_ALWAYS,
    ...
    const int fd = open(g_crash_marker_path, O_CREAT | O_WRONLY | O_APPEND | O_CLOEXEC, 0644);
    if (fd < 0)
    {
        return;
    }
...
LONG WINAPI windows_crash_handler(EXCEPTION_POINTERS* exception_info)
{
    const unsigned long exception_code =
        (exception_info && exception_info->ExceptionRecord)
            ...
            : 0ul;
    char message[96] = {};
    const int message_size = std::snprintf(message,
                                           sizeof(message),
                                           "0x%08lx fatal exception caught by Octaryn\n",
                                           exception_code);
    if (message_size > 0)
    {
        write_marker_bytes(message,
...
void append_signal_number(char* message, std::size_t message_size, int sig)
{
    ...
    for (int i = 2; i >= 0; --i)
    {
        message[i] = static_cast<char>('0' + (value % 10));
        value /= 10;
    }
}
...
void configure_crash_marker_path()
{
    ...
    {
        std::snprintf(g_crash_marker_path, sizeof(g_crash_marker_path), "%s", override_path);
        return;
    }
    ...
    std::snprintf(g_crash_marker_path,
                  sizeof(g_crash_marker_path),
                  "%soctaryn-engine-crash-%lu.marker",
                  base_path,
                  static_cast<unsigned long>(GetCurrentProcessId()));
#else
    std::snprintf(g_crash_marker_path,
                  sizeof(g_crash_marker_path),
                  "/tmp/octaryn-engine-crash-%ld.marker",
...
void oct_crash_diagnostics_init(const char* logger_name)
{
  static std::once_flag once;
  std::call_once(once, [logger_name]() {
    oct_log_init(logger_name);
    configure_crash_marker_path();
    install_signal_handlers();
#if OCTARYN_HAS_CPPTRACE
    cpptrace::register_terminate_handler();
    oct_log_infof("Crash diagnostics initialized with cpptrace terminate handling and marker %s",
                  g_crash_marker_path);
#else
    oct_log_infof("Crash diagnostics initialized with marker %s; cpptrace support is disabled",
                  g_crash_marker_path);
#endif
  });
}

const char* oct_crash_diagnostics_marker_path(void)
{
    return g_crash_marker_path;
}
